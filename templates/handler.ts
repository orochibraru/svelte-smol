import { env_prefix, origin } from "MANIFEST";
...
export async function handler(
	request: Request,
	bun_server: BunServer<undefined>,
): Promise<Response> {
...
function normalize_request(request: Request): Request | Response {
	try {
...
function get_client_address(
	...
	if (!address_header) {
		...
		return bun_server.requestIP(request)?.address as string;
	}
	...
	return (addresses[addresses.length - xff_depth] as string).trim();
}
